...
class OpenAITTSProvider(TTSProvider):
    ...
    Supports models: gpt-4o-mini-tts (current, steerable via instructions),
    plus legacy tts-1 / tts-1-hd. 13 voices; marin and cedar are the
    highest quality (gpt-4o-mini-tts only).
    ...
    def __init__(self, config: dict):
        ...
        self.instructions = self.openai_config.get(
            "instructions"
        )  # Optional instructions parameter
    ...
    def _check_ffmpeg(self) -> bool:
        ...
        try:
            ...
            logger.warning("ffmpeg not found. Audio quality will be degraded (24kHz/128kbps).")
            return False

    def _validate_model(self, model: str) -> str:
        if model not in self.VALID_MODELS:
            logger.warning(f"Invalid OpenAI model '{model}', using 'tts-1'")
            return "tts-1"
        return model

    def _validate_voice(self, voice: str) -> str:
        if voice not in self.VALID_VOICES:
            logger.warning(f"Invalid OpenAI voice '{voice}', using 'nova'")
            return "nova"
        return voice
    ...
    def _validate_format(self, fmt: str) -> str:
        if fmt not in self.VALID_FORMATS:
            logger.warning(f"Invalid format '{fmt}', using 'mp3'")
            return "mp3"
        return fmt
    ...
    def _convert_opus_to_mp3(self, opus_bytes: bytes) -> bytes:
        ...
        try:
            cmd = [
                ...
                opus_path,
                "-ar",
                str(self.hq_sample_rate),  # 44.1kHz
                "-b:a",
                self.hq_bitrate,  # 192kbps
                "-f",
                "mp3",
                mp3_path,
            ]
            ...
            if result.returncode != 0:
                logger.warning("ffmpeg conversion failed, using raw OPUS")
                return opus_bytes
    ...
    def synthesize(self, text: str) -> bytes:
        ...
        use_hq_conversion = self._ffmpeg_available and self.format == "mp3"

        if use_hq_conversion:
            logger.info("  Using high-quality mode: OPUS (48kHz) → MP3 (44.1kHz/192kbps)")
...
def get_voice_description(voice: str) -> str:
    ...
    descriptions = {
        ...
        "ash": "Expressive and dynamic",
        "ballad": "Smooth and expressive (gpt-4o-mini-tts only)",
        "cedar": "Clear and natural - best quality (gpt-4o-mini-tts only)",
        "coral": "Warm and friendly",
        "echo": "Warm and conversational - great for friendly content",
        "fable": "Expressive British storyteller - perfect for narratives",
        "marin": "Clear and professional - best quality (gpt-4o-mini-tts only)",
        ...
        "sage": "Clear and balanced",
        "shimmer": "Soft and gentle - best for calm, meditative content",
        "verse": "Natural and engaging (gpt-4o-mini-tts only)",
    }
