...
def extract_metadata_from_transcript(transcript_text: str) -> dict:
    ...
    """
    metadata = {"duration_seconds": None, "stories": [], "reading_list": [], "custom_title": None}
    ...
    references_match = re.search(
        r"REFERENCES\n-+\n(.+?)(?=\n\nREADING LIST|\n\nPROMPTS USED|$)", transcript_text, re.DOTALL
    )
    ...
    if reading_match:
        ...
        for num, title_line, source, url in entries:
            ...
            metadata["reading_list"].append(
                {"title": title, "author": author, "source": source, "url": url, "description": ""}
            )

    return metadata


def migrate_episode_to_json(
    ...
    episode_data = {
        ...
        "metadata": {"migrated_at": datetime.now().isoformat(), "migration_source": "r2_storage"},
    }
...
def main():
    ...
    response = client.list_objects_v2(Bucket=bucket, Prefix="episodes/", MaxKeys=1000)
    ...
    for episode_id in episode_ids:
        ...
        # Migrate
        episode_data = migrate_episode_to_json(episode_id, config, client, bucket, public_base_url)
