...
from podcast.artwork.brief import generate_art_brief
from podcast.artwork.generator import get_artwork_provider
...
SAMPLE_EPISODES = {
    ...
    "science": [
        MockItem(
            "Scientists discover high-temperature superconductor at room temperature", "Nature"
        ),
...
def main():
    ...
    parser.add_argument(
        "--theme",
        type=str,
        choices=list(SAMPLE_EPISODES.keys()),
        default="mixed",
        help="Sample episode theme",
    )
    parser.add_argument(
        "--local", action="store_true", help="Save locally instead of uploading to R2"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Only generate brief and prompt, no image"
    )
    parser.add_argument(
        "--fetch-real", action="store_true", help="Use real RSS items instead of samples"
    )
    parser.add_argument(
        "--output", type=str, default="test_artwork.png", help="Output filename for --local mode"
    )
    ...
    if args.local:
        ...
        from podcast.storage import upload_artwork_metadata_to_r2, upload_artwork_to_r2
